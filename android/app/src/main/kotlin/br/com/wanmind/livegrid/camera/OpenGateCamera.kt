...
class OpenGateCamera(private val context: Context) {
    ...
    data class OpenResult(val width: Int, val height: Int)
    ...
    fun open(
        ...
        try {
            ...
            val best = pickLargest4x3(sizes) ?: return onError("no 4:3 size")

            manager.openCamera(cameraId, object : CameraDevice.StateCallback() {
                override fun onOpened(camera: CameraDevice) {
                    device = camera
                    createSession(camera, outputs, best, onReady, onError)
                }
        ...
        val cb = object : CameraCaptureSession.StateCallback() {
            override fun onConfigured(s: CameraCaptureSession) {
                ...
                try {
                    ...
                    onReady(OpenResult(size.width, size.height))
                } catch (t: Throwable) {
    ...
    private fun pickLargest4x3(sizes: Array<Size>): Size? {
        return sizes
            .filter { it.height > 0 && it.width > 0 }
            .filter { is4x3(it.width, it.height) }
